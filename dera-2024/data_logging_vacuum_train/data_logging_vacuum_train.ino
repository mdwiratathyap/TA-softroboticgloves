...
unsigned long TimeDuration1 = 15000;
unsigned long TimeDuration2 = 30000;
unsigned long TimeDuration3 = 45000;
unsigned long TimeDuration4 = 60000;
unsigned long TimeDuration5 = 75000;
unsigned long TimeDuration6 = 90000;
unsigned long TimeDuration7 = 105000;
...
void print_header() {
  Serial.println("Time\tPWMA\tPWMB\tAngle\tVolt\tADC\tVref\tVout\tPress");
}
...
void loop() {
  ...
  else if (LastTime1 - StartTime1 < TimeDuration3 and start == true) {
    pwmA = (int) 255*100/100;
    pwmB = (int) 255*pwm/100;
    analogWrite(enA, pwmA);
    analogWrite(enB, pwmB);
    digitalWrite(SolValve[0], LOW);
  ...
  else if (LastTime1 - StartTime1  < TimeDuration4 and start == true) {
    pwmA = (int) 255*50/100;
    pwmB = (int) 255*pwm/100;
    analogWrite(enA, pwmA);
    analogWrite(enB, pwmB);
  ...
  else if (LastTime1 - StartTime1 < TimeDuration5 and start == true) {
    pwmA = (int) 255*70/100;
    pwmB = (int) 255*pwm/100;
    analogWrite(enA, pwmA);
    analogWrite(enB, pwmB);
  ...
  else if (LastTime1 - StartTime1  < TimeDuration6 and start == true) {
    pwmA = (int) 255*90/100;
    pwmB = (int) 255*pwm/100;
    analogWrite(enA, pwmA);
    analogWrite(enB, pwmB);
  ...
  else if (LastTime1 - StartTime1  < TimeDuration7 and start == true) {
    pwmA = 0;
    pwmB = (int) 255*pwm/100;
    analogWrite(enA, pwmA);
    analogWrite(enB, pwmB);
  ...
  if (Time - LastTime >= TimeSampling and start == true) {
    ...
    Serial.print("\t");
    Serial.print((int) (100*pwmB/255.0+0.5));
    Serial.print("\t");
    read_flex();
    read_pressure();
  }
